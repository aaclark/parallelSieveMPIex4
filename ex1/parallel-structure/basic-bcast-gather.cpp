...
int main(int argc, char *argv[]) {
    ...
    if (!rank) { // rank-0 is master
        for (int dest = 0; dest < size; ++dest) { // 1, ... size-1
            ret_vals[dest] = dest; // initialize: "you are number X"
            std::cout << ret_vals[dest] << std::endl;
        }
    }
    MPI_Bcast(ret_vals, size, MPI_INT,
              0, MPI_COMM_WORLD);
     ...
     */
    work_val = do_work(ret_vals[rank]);
     ...
     */
    MPI_Gather(&work_val, 1, MPI_INT,
               &ret_vals[rank], 1, MPI_INT,
               0, MPI_COMM_WORLD);
//    if (!rank) { // rank-0 is master
